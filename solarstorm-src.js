	...
	return declare('bgagame.solarstorm', ebg.core.gamegui, {
		...
		initializeResourceDeck() {
			...
			this.addTooltipMarkdown(
				document.querySelector('.ss-resource-deck__deck'),
				[
					_('Resource deck'),
					'.\n----\n',
				...
				switch (stateName) {
					case 'playerTurn':
						...
						if (args.canRestartTurn) {
							this.showRestartTurnButton(() => {
								this.ajaxAction('restartTurn', { lock: true })
							})
						}
						break
					case 'playerShare':
					...
					case 'playerAskActionTokensPlay':
						...
						this.addActionButton('buttonDontUseToken', _('End turn'), evt => {
							this.ajaxAction('dontUseToken', { lock: true })
						})
						break
				}
			}
		...
		waitForResourceFromDeck(options = {}) {
			// Default options
			options = Object.assign({ table: true, deck: true, cancel: false, restart: false }, options)
			...
			return new Promise((resolve, reject) => {
				...
				if (options.deck) {
					handles.push(
						dojo.connect(document.querySelector('.ss-resource-deck__source'), 'onclick', () => {
							cleanAll()
							resolve({ id: 9999 })
						}),
					)
		...
		waitForPlayerSelection(players, options = {}) {
			...
			return new Promise((resolve, reject) => {
				...
				players.forEach(player => {
					handles.push(
						dojo.connect(player.boardEl, 'onclick', () => {
							cleanAll()
							resolve(player)
						}),
					)
		...
		async doPlayerActionGiveResource(sameRoomOnly = true) {
			...
			try {
				...
				const card = await this.waitForPlayerResource([activePlayer], { cancel: true })
				...
				const player = await this.waitForPlayerSelection(targetPlayers, { cancel: true })
		...
		async doPlayerActionTakeResource(sameRoomOnly = true) {
			...
			try {
				...
				const targetPlayers = this.players
					.getInactive()
					.filter(p => !sameRoomOnly || p.position === activePlayer.position)
				const card = await this.waitForPlayerResource(targetPlayers, { cancel: true })
		...
		async doPlayerActionSwapResource() {
			...
			this.removeActionButtons()
			try {
				const card1 = await this.waitForPlayerResource([this.players.getActive()], { cancel: true })
				this.gamedatas.gamestate.descriptionmyturn = _('Swap cards : You must choose a card to exchange')
				this.updatePageTitle()
				this.removeActionButtons()
				const card2 = await this.waitForPlayerResource(this.players.getInactive(), { cancel: true })
		...
		async doPlayerRoomEngineRoom(cards) {
			try {
				...
				const card2 = await this.waitForPlayerResource([this.players.getActive()], { cancel: true })
		...
		async doPlayerPickResources(possibleSources, canRestartTurn) {
			try {
				...
				if (e === 'RESTART_BTN') {
					await this.ajaxAction('restartTurn', { lock: true })
				} else {
					console.error(e)
				}
			}
